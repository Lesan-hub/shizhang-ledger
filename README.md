...
- 统计追溯：点击支出分类或每日趋势，可直接查看对应账目与备注
- 个性设置：支持昵称、本地头像和主题颜色
- 数据安全：在“我的 → 设置”中导出 CSV 账单或清空账目
...
制作可覆盖旧版并保留本地账目的升级包时，必须沿用旧版包名和签名证书，并提高 `versionCode`。签名配置方式见 [Android 升级签名说明](android/SIGNING.md)；签名文件不得提交到仓库。
